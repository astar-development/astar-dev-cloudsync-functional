...
public interface ISyncPipeline
{
    ...
    Task RunAsync(IEnumerable<SyncJob> jobs, string accessToken, Action<SyncProgressEventArgs> onProgress, Action<JobCompletedEventArgs> onJobCompleted, string accountId, PersistenceDriveId driveId, int workerCount, CancellationToken cancellationToken = default);
}
