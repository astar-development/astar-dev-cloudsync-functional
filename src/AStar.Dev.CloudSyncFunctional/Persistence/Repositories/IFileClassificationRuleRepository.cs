...
public interface IFileClassificationRuleRepository
{
    ...
    Task<IReadOnlyList<FileClassificationRuleEntity>> GetAllAsync(CancellationToken ct = default);
    ...
    Task<Result<Unit, PersistenceError>> UpsertAsync(FileClassificationRuleEntity entity, CancellationToken ct = default);

    /// <summary>Deletes a file classification rule by identifier.</summary>
    /// <param name="id">The rule identifier.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Ok on success, Fail on error.</returns>
    Task<Result<Unit, PersistenceError>> DeleteAsync(string id, CancellationToken ct = default);
}
